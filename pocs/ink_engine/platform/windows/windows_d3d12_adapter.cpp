...
WindowsD3d12Adapter::~WindowsD3d12Adapter() {
  if (impl_->context != nullptr) impl_->context->abandonContext();
}

Status WindowsD3d12Adapter::Initialize(HWND window, bool use_warp,
                                       uint32_t width, uint32_t height) {
  if (width == 0 || height == 0) {
    impl_->error = "D3D12 target dimensions must be positive";
    return Status::kInvalidArgument;
  }
  ComPtr<IDXGIFactory6> factory;
  if (FAILED(CreateDXGIFactory2(0, IID_PPV_ARGS(&factory)))) {
    impl_->error = "CreateDXGIFactory2 failed";
    return Status::kInvalidState;
  }
  if (use_warp) {
    if (FAILED(factory->EnumWarpAdapter(IID_PPV_ARGS(&impl_->adapter)))) {
      impl_->error = "DXGI WARP is unavailable";
      return Status::kInvalidState;
    }
  } else {
  ...
  if (FAILED(impl_->device->CreateCommandQueue(
    ...
    impl_->error = "D3D12 command queue creation failed";
    return Status::kInvalidState;
  }
  ...
  if (impl_->context == nullptr) {
    impl_->error = "Skia failed to create Ganesh D3D12 context";
    return Status::kInvalidState;
  }
  ...
  if (impl_->surface == nullptr) {
    impl_->error = "Skia failed to create D3D12 render target";
    return Status::kInvalidState;
  }
  DXGI_ADAPTER_DESC1 description{};
...
Status WindowsD3d12Adapter::Render(
    const StrokeDocument& document,
    const DefaultPreviewSink::State* preview,
    std::vector<uint8_t>* rgba) {
  if (impl_->surface == nullptr || impl_->context == nullptr) {
    impl_->error = "D3D12 adapter is not initialized";
    return Status::kInvalidState;
  }
  impl_->renderer.Draw(*impl_->surface->getCanvas(), impl_->width,
                       impl_->height, document, preview);
  impl_->context->flushAndSubmit(
      impl_->surface.get(), rgba == nullptr ? GrSyncCpu::kNo : GrSyncCpu::kYes);
  if (rgba != nullptr && !impl_->renderer.Readback(
                             *impl_->surface, impl_->width, impl_->height, rgba)) {
    impl_->error = "D3D12 RGBA readback failed";
    return Status::kInvalidState;
  }
  return Status::kOk;
}

Status WindowsD3d12Adapter::PresentToWindow(std::span<const uint8_t> rgba) {
  if (impl_->window == nullptr) return Status::kOk;
  if (rgba.size() != static_cast<size_t>(impl_->width) * impl_->height * 4U) {
    impl_->error = "D3D12 presentation readback size is invalid";
    return Status::kInvalidArgument;
  }
  ...
  HDC context = GetDC(impl_->window);
  StretchDIBits(context, 0, 0, static_cast<int>(impl_->width),
  ...
  ReleaseDC(impl_->window, context);
  return Status::kOk;
}
...
const std::string& WindowsD3d12Adapter::error() const { return impl_->error; }
