...
bool WindowsImeAdapter::UpdateFromImm(HWND window, LPARAM lparam) {
  ...
  if (lparam & GCS_COMPSTR) {
    const std::string preview = Utf16ToUtf8(CompositionString(context, GCS_COMPSTR));
  ...
  if (lparam & GCS_RESULTSTR) {
    const std::string committed = Utf16ToUtf8(CompositionString(context, GCS_RESULTSTR));
...
bool WindowsImeAdapter::HandleMessage(HWND window, UINT message, WPARAM wparam,
  ...
  switch (message) {
    case WM_SETFOCUS:
      return canvas_poc04_session_focus(session_) == CANVAS_POC04_STATUS_OK;
    case WM_KILLFOCUS:
      composing_ = false;
