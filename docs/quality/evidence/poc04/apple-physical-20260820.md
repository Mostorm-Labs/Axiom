...
controlled `ni hao → 你好` semantic-result gate remains pending. This report
does not change the single unified POC-04 from `Validating` to `Accepted`.
...
| --- | ---: | --- | --- |
| iPhone 15 Pro | 91 | `setMarkedText`, `unmarkText` | yes | yes |
| iPad Air 4 | 78 | `insertText`, `setMarkedText`, `unmarkText` | yes | yes |

The keyboard was visible and the presented text was rendered by the native
editor view from the same C++ session queried by UIKit. The captured event
streams prove that UIKit composition callbacks reach the adapter, update the
same C++ session, and complete a composition. The observed candidate strings
were generated by the active keyboard and were not a controlled `ni hao →
你好` script, so this closes the callback-chain gate but not the controlled
Chinese semantic-result gate.

The follow-up artifacts were collected from the signed bundle after the device
was unlocked:

- iPhone: `validation-iphone-system-input-later.json`, digest
  `690e1c1ac7f50c3372fc41946531587d`.
- iPad: `validation-ipad-system-input.json`, digest
  `bedfb2ff5ca2d70fdfaa8c7b647017e3`, including newline insertions whose
  selection offsets advanced to the next paragraph.

## Remaining gates

- Manually select Chinese Pinyin on each physical device and enter the
  controlled phrase `ni hao` → `你好`, then recover
  `poc04-ime-system-input.json` with the final committed text and Runtime
  digest.
