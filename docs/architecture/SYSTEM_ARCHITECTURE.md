...
flowchart TB
  ...
  subgraph Bridge["Canvas Application API"]
  ...
  subgraph Core["C++20 Canvas Runtime"]
    ...
    Resources["Resources / Persistence"]
    Collab["Collaboration"]
  end

  Ganesh["Skia Ganesh"]
  Surface["WebGL / Native GPU / Headless"]
...
依赖只能自上而下：Shell 依赖 Bridge，Bridge 依赖 Runtime 的公开 facade，Document 不依赖 Editor、Skia、网络或平台，Renderer 不拥有 Document 写入口。
...
- 可见性、dirty region、文本布局引用和资源解析状态。
- 多视口共享的稳定 scene data，以及各视口的查询结果。
...
所有节点具有稳定 ID、局部变换、可见/锁定状态、稳定排序键和版本化属性集合。`Page` 是根，不允许一般节点拥有跨 Page 的隐式父子引用。

### 4.2 扩展类别

```text
Structural: Section / Frame / Group / Table
...
flowchart LR
  ...
  Session --> Preview["Preview Stroke"]
...
class FastInkBackend {
 public:
  ...
  virtual void push(Span<const PointerSample>) = 0;
...
`SceneCompiler` 提供两个逻辑入口：全量 `compile(DocumentSnapshot)` 与增量 `apply(ChangeSet)`。二者输出相同 revision 的 RuntimeScene 时，render records、bounds、hit-test 和视觉结果必须等价。

RuntimeScene 进入以下管线：

1. SpatialIndex 查找视口候选。
2. Visibility 和 clip 过滤不可见内容。
3. Render Tree 解析层级、opacity、mask 和 effect 边界。
4. FrameGraph 构建 Background、Content、Ink、ExternalSurface、Overlay、Selection、HUD passes。
5. Compositor 分配 pass 资源并应用 cache。
6. Skia Ganesh 输出 WebGL、native GPU 或 Headless target。
...
- RuntimeScene 保存 ExternalSurface 的语义 bounds、clip、opacity 和 platform handle reference。
...
POC-01 至 POC-06 默认单线程有序执行 Document、SceneCompiler 和 Renderer，以建立确定性参考结果。接口需传递 revision、不可变 snapshot 和任务取消信息，但不提前创建 worker。
